...
export const premiumProcedure = (entity: "meetings" | "agents") =>
  protectedProcedure.use(async ({ ctx, next }) => {
    ...
    const meetingCount = userMeetings?.count ?? 0;
    const agentCount = userAgents?.count ?? 0;

    // Polar disabled => everyone is treated as free tier
    if (!polarClient || !process.env.POLAR_ACCESS_TOKEN) {
      const isFreeAgentLimitReached = agentCount >= MAX_FREE_AGENTS;
    ...
    // Polar enabled => check subscription
    const customer = await polarClient.customers.getStateExternal({
      externalId: ctx.auth.user.id,
    });
